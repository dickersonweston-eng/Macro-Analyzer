...
namespace {
    std::string getString(matjson::Value const& obj, std::string_view key, std::string const& def) {
        ...
        auto value = obj[key].asString().unwrapOr(def);
...
Replay SchemaParser::makeReplay(matjson::Value const& root, ReplayFileFormat format, std::string const& fileName) {
    Replay replay;
    replay.fileName = fileName;
    replay.fileFormat = format;
